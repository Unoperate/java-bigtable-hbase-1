...
import static com.google.cloud.bigtable.mirroring.hbase1_x.Helpers.createPut;
...
public class TestVerificationSampling {
  ...
  @Mock ReadSamplingStrategy readSamplingStrategy;
  ...
  public void isGetSampled() throws IOException {
    ...
    verify(readSamplingStrategy, times(1)).shouldNextReadOperationBeSampled();
    ...
    verify(readSamplingStrategy, times(2)).shouldNextReadOperationBeSampled();
  ...
  private void withSamplingEnabled(boolean b) {
    doReturn(b).when(readSamplingStrategy).shouldNextReadOperationBeSampled();
  }
}
